...
class InferenceCore:
    ...
    def do_pass(self, key_k, key_v, idx, forward=True, step_cb=None):
        ...
        for ti in this_range:
            ...
            if (closest_ti != self.t) and (closest_ti != -1):
                ...
                print("self.prob.shape:", self.prob.shape)
                out_mask = out_mask[:self.prob.shape[0]]  # keep only as many masks as prob expects
    ...
    def interact(self, mask, idx, total_cb=None, step_cb=None):
        ...
        #key_k, key_v = self.prop_net.memorize(self.get_image_buffered(idx), mask)
